...
SslTool::SslTool()
{
    BIO* bio = BIO_new_mem_buf(SERVER_PUBLIC, -1);      //openssl提供了bio接口以支持各种形式的秘钥读取
    if(bio == NULL) //读取密钥失败
    {
        throw "load public key failed!";
    }
    rsa_public = PEM_read_bio_RSA_PUBKEY(bio, NULL, NULL, NULL);  //获取加密公钥
    BIO_free(bio);  //bio是读取密钥用的，主要用在了读取加密密钥上，用完之后就可以释放掉了，以备下一次使用（公钥读完还是私钥）
    bio = BIO_new_mem_buf(SERVER_PRIVATE, -1);  //读取私钥
    if(bio == NULL)
    {
        throw "load private key failed!";
    }
    rsa_private = PEM_read_bio_RSAPrivateKey(bio, NULL, NULL, NULL); //获取加密私钥
}
...
string BigNum::base64Encode(const string& buffer, bool newline)
{
    ...
    bmem = BIO_new(BIO_s_mem());    //BIO_s_mem:返回内存bio方法的函数 BIO_new:新分配一个BIO
    b64 = BIO_push(b64, bmem);
    BIO_write(b64, buffer.c_str(), (int)buffer.size()); //向这个BIO写入数据
    BIO_flush(b64); //刷新,通常写出任何内部缓冲数据，在某些情况下，它用于发出 EOF 信号，并且不会再写入更多数据。
...
QByteArray SslTool::rsaEncode(const QByteArray& buffer)
{
    ...
    result.resize(buffer.size() * 3);//因为需要对齐，所以可能会长很多
    ...
    int ret = RSA_public_encrypt(
                  ...
                  rsa_public, RSA_PKCS1_PADDING);                       //加密用的rsa公钥
...
QByteArray SslTool::rsaDecode(const QByteArray& input)
{
    ...
    int ret = RSA_private_decrypt(
                  ...
                  (unsigned char*)result.data(),
                  rsa_private, RSA_PKCS1_PADDING);                      //解码用的rsa私钥         //:问老师，why？一个公钥一个私钥？
                                //一般客户端使用公钥加密后，发给服务端数据，这个过程中，就算客户端被破解了，他拿到了公钥，
                                //数据包也被拦截了，他也不能解密，因为解密需要私钥，私钥在服务端，所以相对来说安全一些
    if(ret == -1)
    {
        result.clear();
        return result;
...
BigNum::BigNum()
{
    bignum = BN_new(); //构造函数，new出来一片新空间()BN_new 分配并初始化 BIGNUM 结构
}
...
BigNum::BigNum(const string& bm, bool is_base64/* = true*/)
{
    ...
    // 将 s 处长度 len 的大端形式的正整数转换为 BIGNUM，并将其置于 ret。如果 ret 为 NULL，则会创建一个新的 BIGNUM。
}
