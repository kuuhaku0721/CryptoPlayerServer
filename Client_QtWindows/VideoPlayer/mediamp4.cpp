...
MediaMP4::MediaMP4()                        //打完注释忘保存了
{
    //TODO:这里要处理一下明文
    //QFile f(":/ico/UI/ico/128-128.png");
    QFile f(":/ico/UI/ico/key.png");
...
int MediaMP4::open(const QUrl& url, uint64_t* fsize)
{
    ...
    if(file->open(QFile::ReadOnly))
    {
        *fsize = file->size();
        ...
        //qDebug() << __FUNCTION__  << "(" << __LINE__ << ")" << "pos:" << pos;
...
ssize_t MediaMP4::read(uint8_t* buffer, size_t length)
{
    ...
    //qDebug() << __FUNCTION__  << "(" << __LINE__ << ")" << "len:" << len;
    //qDebug() << __FUNCTION__  << "(" << __LINE__ << ")" << "length:" << length;
    //qDebug() << __FUNCTION__  << "(" << __LINE__ << ")" << "position:" << position;
    if(len > 0)
    {
